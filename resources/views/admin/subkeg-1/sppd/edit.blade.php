...
@section('content')
    <main class="container py-4">
        ...
        @endif
        <form action="{{ route('subkeg-1.sppd.update', $sppd->id) }}" method="POST" enctype="multipart/form-data">
            @method("PUT")
            @csrf
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="pegawai_pemberi_wewenang" id="pegawai_pemberi_wewenang" required>
                    ...
                    @foreach ($pegawai as $p)
                        <option value="{{ $p->id }}" {{ $sppd->pegawai_pemberi_wewenang == $p->id ? 'selected' : ''}}>{{ $p->nip }} - {{ $p->nama }} - {{ $p->jabatan }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" name="pegawai_pelaksana" id="pegawai_pelaksana" required>
                    <option value="" selected disabled>-- Pilih Pegawai --</option>
                    @foreach ($pegawai as $p)
                        <option value="{{ $p->id }}" {{ $sppd->pegawai_pelaksana == $p->id ? 'selected' : ''}}>{{ $p->nip }} - {{ $p->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="pegawai_pengikut_1" id="pegawai_pengikut_1">
                    <option value="" selected disabled>-- Pilih Pegawai --</option>
                    @foreach ($pegawai as $p)
                        <option value="{{ $p->id }}" {{ $sppd->pegawai_pengikut_1 == $p->id ? 'selected' : ''}}>{{ $p->nip }} - {{ $p->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" name="pegawai_pengikut_2" id="pegawai_pengikut_2">
                    <option value="" selected disabled>-- Pilih Pegawai --</option>
                    @foreach ($pegawai as $p)
                        <option value="{{ $p->id }}" {{ $sppd->pegawai_pengikut_2 == $p->id ? 'selected' : ''}}>{{ $p->nip }} - {{ $p->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" name="pegawai_pengikut_3" id="pegawai_pengikut_3">
                    <option value="" selected disabled>-- Pilih Pegawai --</option>
                    @foreach ($pegawai as $p)
                        <option value="{{ $p->id }}" {{ $sppd->pegawai_pengikut_3 == $p->id ? 'selected' : ''}}>{{ $p->nip }} - {{ $p->nama }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
